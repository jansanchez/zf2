...
$.TextboxList = function(element, _options){
	...
	var init = function(){		
            ...
            for (var name in options.plugins) {
                enablePlugin(name, options.plugins[name]);
            };		
            afterInit();
	};
		...
		$(document).click(function(e){
                        ...
                        if (e.target.className.indexOf(options.prefix) != -1){				
                            ...
                            var parent = $(e.target).parents('div.' + options.prefix);
                                    if (parent.get(0) == container.get(0)) {
                                        return;
                                    }
                        }
                        blur();
	...
	var getValues = function(){
		...
		list.children().each(function(){
			var bit = getBit(this);
			if (!bit.is('editable')) labels.push(bit.getLabel());
		});
            return labels;
        };
	...
	var init = function(){
		...
		if (type == 'box'){
			...
			if (options.deleteButton){
				...
				close = $('<a href="#" class="'+ typeprefix +'-deletebutton" />').mousedown(function(evt){
                                    evt.stopPropagation(); 
                                    evt.preventDefault();
                                }).mouseup(remove).appendTo(bit);
			}
	...
	this.setValue = function(v){
 	...
 	this.getValue = function(){
        ...
        this.getLabel = function(){
            return type == 'editable' ? [null, element.val(), null] : value[1];
        };
        /*
	if (type == 'editable'){
