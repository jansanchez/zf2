...
var peruid = function(par) {
    ...
    $('body').append('<div id="peruid_modal"><div class="pid_back"></div><div class="pid_frame" style="top:-800px; height:1px; width:1px"><a class="pid_close" href="#cerrar">Cerrar</a><iframe allowtransparency="true" id="pid_iframe" width="510px" height="284px" frameborder="0" scrolling="no"></iframe></div></div> ');
    ...
    t.add_modal = function(o) {
        ...
        o.each(function() {
            var _o = $(this);
            var rd = _o.data('redirect');
            rd = rd ? ('&redirect=' + encodeURIComponent(rd)) : '';
            var erd = _o.data('end-redirect');
            erd = erd ? ('&end_redirect=' + encodeURIComponent(erd)) : '';

            if (t.mobile) {
                if (this.href.indexOf('/login/') > 0)
                    _o.attr('href', par.path_base + 'movil/login/' + par.path_portal + '?callback=' + encodeURIComponent(window.location.pathname) + rd + erd);
                else
                    _o.attr('href', par.path_base + 'movil/registro/' + par.path_portal + '?callback=' + encodeURIComponent(window.location.pathname) + rd + erd);
            } else {
                _o.click(function(e) {
                    e.preventDefault();
                    var s = size();
                    ...
                    //back frame
                    m.find('iframe').css({width: 1, height: 1}).end().find('div.pid_back').css({width: s[0] + 'px', height: s[1] + 'px'}).end().show();

                    //server
                    if (this.href.indexOf('/registros/redesociales/portada') > 0)
                        m.find('iframe').attr('src', par.path_base + 'registros/redesociales/' + par.path_portal + '?callback=' + document.location.pathname + '&proxy=' + encodeURIComponent(par.path_proxy) + rd + erd);
                    else if (this.href.indexOf('/registro/') > 0)
                        m.find('iframe').attr('src', par.path_base + 'registros/registrate/' + par.path_portal + '?callback=' + document.location.pathname + '&proxy=' + encodeURIComponent(par.path_proxy) + rd + erd);
                    else
                        m.find('iframe').attr('src', par.path_base + 'registros/loggin/' + par.path_portal + '?callback=' + document.location.pathname + '&proxy=' + encodeURIComponent(par.path_proxy) + rd + erd);
                    if (!t.showed)
                        t.center();
                });
            }
    ...
    t.proxy = function(_p) {
        ...
        if (_a["type"] == "resize")
            t.center({height: _a["height"], width: _a["width"]});
        else if (_a["type"] == "hashing")
            t.hashing({hash: _a["tk"]});
        else if (_a["type"] == "reload")
            window.location.reload();
        else if (_a["type"] == "redirect")
            window.location.replace(_a["to"]);
        else if (_a["type"] == "end_redirect") {
            if ($.trim(_a["to"]) == "")
                t.close_modal();
            else
                t.end_redirect = _a["to"];
        }
    }
    t.close_modal = function() {
        m.hide().find('#pid_iframe').attr('src', '');
        if ($.trim(t.end_redirect) != "")
            window.location.replace(t.end_redirect);
    }
            ...
            function(js) {
                ...
                function(js) {
                    ...
                    if ((js.usuario !== null) && js.sesion) {
                        ...
                        document.cookie = 'pid_anonymous=; expires=Thu, 01 Jan 1970 00:00:01 GMT; path=/';
                        _c = 1;
                        _cc = 'Solo existe sesion en peruid, inicio forzado';
                    } else if ((js.usuario !== null) && js.sesion == false) {
                        t.logued = false;
                        document.cookie = 'pid_token=; expires=Thu, 01 Jan 1970 00:00:01 GMT; path=/';
                        _c = 3;
                        _cc = 'Solo existe sesion en 2do servidor, logout forzado';
                    } else if ((js.usuario == null) && js.sesion == false) {
                        t.logued = false;
                        document.cookie = 'pid_token=; expires=Thu, 01 Jan 1970 00:00:01 GMT; path=/';
                        _c = 4;
                        _cc = 'No existe sesion en ambos servidores';
                    } else {
                        t.logued = true;
                        document.cookie = 'pid_anonymous=; expires=Thu, 01 Jan 1970 00:00:01 GMT; path=/';
                        _c = 2;
                        _cc = 'Existe sesion en ambos servidores';
                    }
                    if (typeof par.callback == 'function')
                        par.callback({o: t, j: js, code: _c, msg: _cc});
                }, "json"
                        ).fail(function() {
    ...
    css.rel = 'stylesheet';
    css.media = 'screen';
    css.href = par.path_base + "f/css/modal.css?v=v201209";
